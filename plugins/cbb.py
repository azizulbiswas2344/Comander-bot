...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    elif data.startswith("generatestreamlink"):
        _, lazydev_id = data.split("#")  # Extract the base64string
        try:
            ...
            await query.message.edit_text(
                ...
                reply_markup=InlineKeyboardMarkup([
                    ...
                    [
                        InlineKeyboardButton("<> Get EMBED code </>", callback_data=f"getembedcode#{lazydev_id}")
                    ]
                    ])  # web stream Link
            )
        except Exception as e:
    ...
    elif data.startswith("getembedcode"):
        _, lazydev_id = data.split("#")  # Extract the base64string
        try:
            xo = await query.message.reply_text(f'🔐')
